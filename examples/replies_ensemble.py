...
conv = keras.layers.Conv1D(HIDDEN, 3, activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(embedded)
conv = keras.layers.MaxPooling1D(3)(conv)
conv = keras.layers.Conv1D(HIDDEN, 3, activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(conv)
conv = keras.layers.MaxPooling1D(3)(conv)
conv = keras.layers.Conv1D(HIDDEN, 3, activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(conv)
conv = keras.layers.MaxPooling1D(3)(conv)
...
recurrent_forward = keras.layers.CuDNNLSTM(HIDDEN, kernel_regularizer=keras.regularizers.l2(0.))(conv)
recurrent_backward = keras.layers.CuDNNLSTM(HIDDEN, kernel_regularizer=keras.regularizers.l2(0.))(mailscanner.layers.TimeStepReverse()(conv))
recurrent = keras.layers.Concatenate()([recurrent_forward, recurrent_backward])

# recurrent with attention, this generates sequences
# note this is GPU only, and keras
# '>=2.0.9, it is shocking slow otherwise
recurrent_sequence_forward = keras.layers.CuDNNLSTM(HIDDEN, return_sequences=True, kernel_regularizer=keras.regularizers.l2(0.))(conv)
recurrent_sequence_backward = keras.layers.CuDNNLSTM(HIDDEN, return_sequences=True, kernel_regularizer=keras.regularizers.l2(0.))(mailscanner.layers.TimeStepReverse()(conv))
...
self_attention = mailscanner.layers.TimeDistributedSelfAttention(activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(recurrent_sequence)
time_attention = mailscanner.layers.SelfAttention(activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(recurrent_sequence)
...
stack = keras.layers.Dense(HIDDEN, activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(ensemble)
stack = keras.layers.Dropout(0.5)(stack)
stack = keras.layers.Dense(HIDDEN, activation=ACTIVATION, kernel_regularizer=keras.regularizers.l2(0.))(stack)
stack = keras.layers.Dropout(0.5)(stack)
